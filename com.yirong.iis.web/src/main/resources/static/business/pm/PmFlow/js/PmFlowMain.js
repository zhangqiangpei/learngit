    ...
    methods: {
        ...
        ableClick : function(){
        	...
        	if(ids.length == 0){
        		...
        		ak.warning("请选择单条记录进行停用!");
        	}else{
        		ak.confirm("确定要停用选中的记录吗？",this.ableCallback);
        	}
        },
        //停用回调函数
        ableCallback : function(instance){
        	...
        	//请求后台--单条任务停用
        	var result = ak.msService("pm","PmFlowApi/able",{ id : idsStr});
        	if(result.code == 0){//操作成功
            	//提示
            	ak.success(result.msg);
            	//刷新表格
      ...
      //共享资源启动任务
        shareRunClick : function(){
        	//获取表格选中的id集
        	var ids = this.tableGetCheckIds();
        	ak.confirm("确定要共享资源启动/停用选中的【" + ids.length + "】条记录吗？",this.sharCallback);
        },
        sharCallback :function(instance){
        	//处理参数
        	var ids = this.tableGetCheckIds();
        	var idsStr = "";
        	$.each(ids,function(i,n){
