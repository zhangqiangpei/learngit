    ...
    methods: {
        ...
        downFile :function(){
        	...
        	if(ids.length != 1){
        		...
        		var idStr =  detailMainVue.tableGetCheckObjs()[0].id;
        		var result = ak.msService("ct","CtManualDataDetailApi/getSuffix",{ id : idStr});
        		if(result.code==0){
        			var suffix = detailMainVue.tableGetCheckObjs()[0].suffixName; 
...
var dialogAttr = getDialogMergeAttr({
	el: '#CtManualDataDetailMain',
	//页面初始化事件
    mounted: function () {
    	//初始化table
    	this.tableInit("ct","CtManualDataDetailApi/list");
    	this.tableSearchModel.manualDataId = mainVue.tableGetCheckObjs()[0].id;
    	//请求后台
    	this.searchClick();
    	//加载初始化数据
    }
...
//生成vue对象
var detailMainVue = ak.getMergeVue(dialogAttr,tableAttr);