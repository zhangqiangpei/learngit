    ...
    methods: {
        update: function () {
            this.$refs["updateModel"].validate(function (valid) {
                if (valid) {//验证通过
                    var result = ak.msService("mm", "IisCountrySurveyApi/update", updateVue.updateModel);
...
var mainAttr = {
    el: '#update',
    mounted: function () {
        var result = ak.msService("mm", "IisCountrySurveyApi/get", {id: detailVue.updateSurveyId});
        if (result.code === 0) {
